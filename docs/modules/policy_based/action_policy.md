...
**Note:** For policy-based modules like ActionPolicy, `:view_only` and `:edit_only` options behave the same as `true` since ActionPolicy uses a single `authorize_with` for both viewing and editing.
